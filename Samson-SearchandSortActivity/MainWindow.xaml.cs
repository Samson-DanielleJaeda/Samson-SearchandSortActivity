...
namespace List3
{
    public partial class MainWindow : Window
    {
        ...
        private void BtnLinearSearch_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                lblResult.Text = "Please enter an item name.";
                return;
            }

            List<ItemInformation> searchResults = new List<ItemInformation>();

            foreach (ItemInformation item in items)
            {
                if (item.ItemName.ToLower().Contains(searchName.ToLower()))
                {
            ...
            {
                ItemGrid.ItemsSource = null;
                ItemGrid.ItemsSource = searchResults;
            }
            else
            {
                ItemGrid.ItemsSource = null;
